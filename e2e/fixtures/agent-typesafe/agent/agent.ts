...
export default defineAgent({
  ...
  model: fixtureModel((request) => {
        ...
        toolCalls: [
          {
            name: "decide",
            input: {
              state: "Alice reproduced the reported failure.",
              questions: {
                reproduced: {
                  type: "probability",
                  prompt: "Does the evidence describe a reproduced failure?",
                },
              },
            },
          },
    ...
    return JSON.stringify({ ...routing.get(), decision: request.toolResults.at(-1)?.output });
  }),
});
