...
For an agent-only workspace, run `eve build`. eve owns the Vercel Build Output and generates one independently built service and an `/eve/<name>/v1/*` transport route for each workspace member.
